...
library(knitr)
for (i in seq(1,length(studentHist$count)-1) ) {
  t<-combine_words(c(studentHist$breaks[i],studentHist$breaks[i+1]),and = "," )
  #paste0(studentHist$counts[i]," students in range (", t,"]"  )
  #    sprintf(studentHist$counts[i]," students in range (", as.character(t),"]"  )
  sprintf("%d students in range (,%s ] ", studentHist$counts[i],t)
}
